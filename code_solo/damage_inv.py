...
def experience():
	...
	threshold1 = parser.get3("data/threshold1_inv.txt")
	threshold2 = parser.get3("data/threshold2_inv.txt")
	
	for k in range(MAKEREF):
		print(k)
		frf_damaged, acc = frf_multiple_sensors(2, acc)
		error_damaged = itp.global_error(minus_list(frf_ref,frf_damaged))
		#diff = minus_list(error_damaged, error_ref)
		for x in range(CAPTVALUE):
			if error_damaged[x] > threshold1[x]:
				detect_damage1[x] = detect_damage1[x] + 1
			if error_damaged[x] > threshold2[x]:
				detect_damage2[x] = detect_damage2[x] + 1
	
	parser.writeValues3("data/detect_damage_1_inv_"+str(DAMAGE)+"percent.txt",detect_damage1)
	parser.writeValues3("data/detect_damage_2_inv_"+str(DAMAGE)+"percent.txt",detect_damage2)
	print("Detect_damage writed in data/detect_damage_i_inv_"+str(DAMAGE)+"percent.txt")
