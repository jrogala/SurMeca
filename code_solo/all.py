...
def frf_multiple_sensors(inFileRef, inFileInf, inFileSup, inFileDamage,remake=True,damage=DAMAGED_SENSOR):
	...
	"""
	for i in range(CAPTVALUE):
		res.append(frf.frf(tool.get_lines(f_damaged,i),tool.get_lines(measurement_damaged,i)))
	parser.writeValues(inFileDamage,res)
...
def experience():
	...
	for i in range(n):
		if (i%10 == 0):
			...
			plt.plot(np.absolute(ref[i]), label = "Reference")
			plt.xlabel('Sensors')
			plt.ylabel('FRF')
