...
def frf_theo(freq, col, M, K, C):
	...
	res = [ 0 for k in range(CAPTVALUE) ]
...
def frf_multiple_sensors(inFileRef, inFileInf, inFileSup, inFileDamage,damage=DAMAGED_SENSOR,d=0):
	...
	for j in range(CAPTVALUE):		
		...
		freq = []
		for i in range(CAPTVALUE):
			v_frf, f_frf = frf(tool.get_lines(f,j),tool.get_lines(measurement,i),sampling_freq)#[k] with MAKEREF
			tmp.append(v_frf)
			freq.append(f_frf)
		if (j == 0):
			test_v = list(tmp)
	...
	for j in range(CAPTVALUE):		
		...
		freq = []
		for i in range(CAPTVALUE):
			v_frf, f_frf = frf(tool.get_lines(f_damaged,j),tool.get_lines(measurement_damaged,i),sampling_freq)#[k] with MAKEREF
			tmp.append(v_frf)
			freq.append(f_frf)
		if (j == 0):
			test_v = list(tmp)
...
filename = "data/stats_"
if(0):
	make_stat(filename)

av_undamaged = parser.get3(filename+"avu.txt")
std_undamaged = parser.get3(filename+"stdu.txt") 
av_damaged = parser.get3(filename+"avd.txt")
std_damaged = parser.get3(filename+"stdd.txt")
