...
These techniques may be costly, taking a long time to be performed and impractical to detect damage in large urban areas.
 Furthermore, they may fail if damage is not visibly evident. This is why other global methods were developed that are automated
 and based on sensor measurements.
...
Figure (\ref{frf_freq}) present results of the FRF on the simulation datas: here is the curve of the FRF (absolute value) for one sensor, in function of frequencies.


\begin{figure}
  \centering
  \includegraphics[width=0.3\textwidth]{images/FRF_freq.png}
  \caption{FRF for one sensor, in function of frequencies}
  \label{frf_freq}
\end{figure}
