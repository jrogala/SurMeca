...
Figure (\ref{damage}) is the FRF values for ten sensors (at a fixed frequency). Here, we easily can see that there is a damage between the sensors 4 and 5 (as we noticed it in the simulation). However, 
...
Figure (\ref{damage2}) is the FRF values for the same ten sensors that in Figure (\ref{damage}), at a different frequency.
...
Finaly, we can see in Figure (\ref{globalerror}) the global error described in section IV.
