...
def simulation(typ,acc):
	...
	if (typ == 1):#no damage
		...
		measurement_ref = m
		av_noise, std_noise = av_std_for_noise(measurement_ref)
		for i in range(SAMPLVALUE):
			for j in range(CAPTVALUE):
				measurement_ref[i][j] = measurement_ref[i][j] + 0.01*norm.rvs(size=1, loc = 0, scale = std_noise)[0]
	...
	if (typ == 2):#damage
		...
		av_noise, std_noise = av_std_for_noise(measurement_damaged)
		for i in range(SAMPLVALUE):
			for j in range(CAPTVALUE):
				measurement_damaged[i][j] = measurement_damaged[i][j] + 0.01*norm.rvs(size=1, loc = 0, scale = std_noise)[0]
...
def experience():
	...
	threshold1 = parser.get3("data/threshold1_noise.txt")
	threshold2 = parser.get3("data/threshold2_noise.txt")
	threshold1_inv = parser.get3("data/threshold1_inv_noise.txt")
	threshold2_inv = parser.get3("data/threshold2_inv_noise.txt")
	...
	parser.writeValues3("data/detect_damage_1_"+str(DAMAGE)+"percent_noise.txt",detect_damage1)
	parser.writeValues3("data/detect_damage_2_"+str(DAMAGE)+"percent_noise.txt",detect_damage2)
	parser.writeValues3("data/detect_damage_1_inv_"+str(DAMAGE)+"percent_noise.txt",detect_damage1_inv)
	parser.writeValues3("data/detect_damage_2_inv_"+str(DAMAGE)+"percent_noise.txt",detect_damage2_inv)
	print("Detect_damage writed in data/detect_damage_i_"+str(DAMAGE)+"percent_noise.txt")
