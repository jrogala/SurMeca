...
def frf_multiple_capter_simu_undamaged(inFile=None, fichier_sampl = "f_sampl_undamaged.txt"):
    ...
    acc, measures, f_sampl = simu.simulation(pos,mass,stiffness,forces,"F_undamaged.txt","Y_undamaged.txt", fichier_sampl)
    #donnes de la simu
...
def frf_multiple_capter_simu_damaged(damage=5,inFile=None, fichier_sampl = "f_sampl_damaged.txt"):
    ...
    #simu: a activer seulement si on veut de nouvelles donnees
    forces = simu.white_noise(SAMPLVALUE,CAPTVALUE)
    acc, measures, f_sampl = simu.simulation(pos,mass,stiffness,forces,"F_damaged.txt","Y_damaged.txt", fichier_sampl, damaged = "f_sampl_ref.txt")
    #donnes de la simu
...
def frf_multiple_capter_simu_ref(inFile=None, fichier_sampl = "f_sampl_ref.txt"):
    ...
    #acc, measures, f_sampl = simu.simulation(pos,mass,stiffness,forces,"F_ref.txt","Y_ref.txt", fichier_sampl)
    ...
    for i in range(10):
    	...
    	#simu: a activer seulement si on veut de nouvelles donnees
    	forces = simu.white_noise(SAMPLVALUE,CAPTVALUE)
    	acc, m, f_sampl = simu.simulation(pos,mass,stiffness,forces,"F_ref.txt","Y_ref.txt", fichier_sampl)
